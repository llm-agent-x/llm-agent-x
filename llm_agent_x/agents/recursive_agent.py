...
class TaskLimitConfig:
    ...
    @staticmethod
    def falloff(initial_tasks: int, max_depth: int, falloff_func: Callable[[int], int]) -> List[int]:
...
class TaskLimit(BaseModel):
    ...
    @classmethod
    def from_falloff(cls, initial_tasks: int, max_depth: int, falloff_func: Callable[[int], int]):
        return cls(limits=TaskLimitConfig.falloff(initial_tasks, max_depth, falloff_func))
...
class RecursiveAgent():
    ...
    def run(self) -> str:
        if self.options.pre_task_executed:
            self.options.pre_task_executed(task=self.task, uuid=self.uuid, 
    ...
    def _run_single_task(self) -> str:
        ...
        for tool_call in response.tool_calls:
            try:
                ...
                history.append(ToolMessage("Tool not found", tool_call_id=tool_call["id"]))
                self.options.on_tool_call_executed(task=self.task, uuid=self.uuid, tool_name=tool_call["name"], tool_args=tool_call["args"], tool_response=tool_response, success=False)
