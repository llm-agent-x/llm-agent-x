...
class TestExecPythonSandbox(unittest.TestCase):
    ...
    @patch('llm_agent_x.tools.exec_python.requests.post')
    @patch('builtins.open', new_callable=unittest.mock.mock_open, read_data="pickle data")
    @patch('os.path.basename', return_value="my_data.pkl")
    def test_3_upload_and_load_pickle_and_execute(self, mock_basename, mock_open_file, mock_post):
        # Simulate /upload, then /load_pickle, then /execute
        mock_upload_response = mock_sandbox_response(200, {"message": "Pickle uploaded"})
        mock_load_pickle_response = mock_sandbox_response(200, {"message": "Pickle loaded"})
        mock_execute_response = mock_sandbox_response(
            200, {"stdout": "Pickle value: test", "stderr": "", "error": None}
        )
        mock_post.side_effect = [mock_upload_response, mock_load_pickle_response, mock_execute_response]
        ...
        mock_post.assert_any_call(
            "http://localhost:5000/load_pickle", json={"file_path": pickle_file_sandbox_path}
        )
        # Check execute call
        mock_post.assert_any_call(
            "http://localhost:5000/execute", json={"code": code}
    ...
    def test_4_sandbox_api_upload_file_not_found(self, mock_open, mock_post):
        ...
        self.assertTrue("Error: File not found for upload: non_existent_file.txt" in result["stderr"])
    ...
    def test_5_sandbox_api_pickle_load_error(self, mock_post):
        ...
        error_response_mock.json.return_value = {"error": "Pickle decode error", "trace": "some traceback"}
        ...
        self.assertTrue("Pickle decode error" in result["stderr"]) # Check for sandbox error message
    ...
    def test_6_sandbox_api_execute_error(self, mock_post):
        ...
        error_response_mock = MagicMock()
        error_response_mock.status_code = 500
        error_response_mock.json.return_value = {"stdout": "", "stderr": "Syntax Error!", "error": "Execution failed", "trace": "detail trace"}
        error_response_mock.raise_for_status.side_effect = requests.exceptions.HTTPError(response=error_response_mock)
        ...
        self.assertTrue("Execution error from sandbox: Execution failed" in result["stderr"])
    ...
    def test_9_sandbox_url_not_configured(self, mock_requests_post, mock_getenv):
        ...
        self.assertEqual(result["stderr"], "PYTHON_SANDBOX_API_URL environment variable is not set.")
    ...
    @patch('builtins.open', new_callable=unittest.mock.mock_open, read_data="file content")
    @patch('os.path.basename', return_value="testfile.txt")
    def test_10_sandbox_api_upload_request_fails(self, mock_basename, mock_open, mock_post):
        # Simulate an error during the requests.post call for upload
        mock_post.side_effect = requests.exceptions.RequestException("Simulated network error during upload")

        result = exec_python(
            "print('test')",
            use_docker_sandbox=True,
            files_to_upload=["existent_file.txt"] # File assumed to be successfully opened by mock_open
        )

        mock_open.assert_called_once_with("existent_file.txt", 'rb')
        mock_post.assert_called_once() # Ensure requests.post was attempted
        self.assertTrue("Error uploading existent_file.txt: Simulated network error during upload" in result["stderr"])
        self.assertEqual(result["error"], "File upload error")
