...
class AlgorithmicMerger:
    def __init__(self, options: MergeOptions):
        ...
        self.model = SentenceTransformer(MODEL_NAME) # Load sentence transformer model here

    def _preprocess_text(self, text):
        # Normalize whitespace and handle paragraph breaks better
        text = re.sub(r'\n\s*\n', '\n\n', text) # Normalize multiple newlines to double
        text = re.sub(r'(?<!\n)\n(?!\n)', ' ', text) # Single newlines (not paragraph breaks) to space
        return text.strip()
    ...
    def _segment_into_sentences(self, text):
        ...
        for para_idx, para in enumerate(paragraphs):
            if not para.strip():
                if para_idx > 0 and para_markers and para_markers[-1] == len(all_sentences) -1:
                    # Avoid double paragraph markers if previous sentence ended a para
                    pass
                elif all_sentences: # Only add a paragraph marker if there are preceding sentences
                     para_markers.append(len(all_sentences) -1) # Mark end of previous para
                continue

            sentences = nltk.sent_tokenize(para.strip())
            if sentences:
                all_sentences.extend(sentences)
                para_markers.append(len(all_sentences) - 1) # Mark the last sentence of this paragraph
    ...
    def _merge_two_texts(self, text1_raw, text2_raw,
        ...
        sents2, _ = self._segment_into_sentences(text2_processed) # Para markers for text2 less critical for this strategy
        ...
        unmatched_sents2 = [] # Sentences from text2 that don't meet insertion_threshold
        ...
        if len(sents2) > 0 and len(sents1) > 0: # Ensure embeds2 is not empty
            similarity_matrix = cosine_similarity(embeds2, embeds1) # rows: sents2, cols: sents1
            ...
            for i2, sent2 in enumerate(sents2):
                ...
                if best_match_score >= insertion_threshold:
                    insertion_map[best_match_s1_idx].append((i2, sent2, best_match_score))
                else:
                    unmatched_sents2.append(sent2)
        ...
        for i1, sent1 in enumerate(sents1):
            ...
            sents2_to_insert = sorted(insertion_map.get(i1, []), key=lambda x: x[0]) # x[0] is original_index_in_sents2
            ...
            if i1 in para_markers1 and i1 < len(sents1) -1 : # Don't add for the very last sentence
        ...
        if unmatched_sents2:
            ...
            for sent2 in unmatched_sents2:
                ...
                for i_s2, s_s2_text in enumerate(sents2):
                    if s_s2_text in unmatched_sents2 and i_s2 not in used_sents2_indices:
                        original_indices_of_unmatched.append(s_s2_text)

                if original_indices_of_unmatched: # If there are truly unmatched and unused sentences
                    if merged_sentences and not merged_sentences[-1].endswith("\n\n"):
        ...
        for i, part in enumerate(final_text_parts):
            if part == "\n\n":
                output_text = output_text.rstrip(" ") # Remove space before paragraph break
                output_text += "\n\n"
            else:
                output_text += part
