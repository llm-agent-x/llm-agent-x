...
import { Play, Pause, X, Send, Edit, MessageSquareQuoteIcon } from 'lucide-react'; // Import // app/components/TaskInspector.tsx
...
export const TaskInspector = ({ task }: { task: any | null }) => {
  ...
  return (
    <div className="bg-zinc-800/50 p-4 rounded-lg border border-zinc-700 h-full overflow-y-auto">
      ...
      <dl>
        <DetailRow label="Dependencies" value={depsArray.length > 0 ? <pre className="font-mono text-xs">{depsArray.join('\n')}</pre> : 'None'} />
        <DetailRow label="Result" value={
            <pre className="whitespace-pre-wrap font-sans bg-zinc-900/80 p-2 rounded-md max-h-48 overflow-y-auto">{task.result || 'Not available'}</pre>
        } />
        {task.human_directive && (
            <DetailRow label="Active Human Directive" value={
...
export const CommandPalette = ({ taskId, taskStatus, currentQuestion }: { taskId: string; taskStatus: string; currentQuestion: any | null; }) => {
      ...
      {taskStatus === 'waiting_for_user_response' && currentQuestion ? (
        <div className="bg-orange-900/30 border border-orange-700 p-3 rounded-md mb-4 animate-pulse">
          <div className="flex items-center gap-2 mb-2 text-orange-200">
            ...
            <span className="font-semibold">Agent Question (Priority: {currentQuestion.priority}/10)</span>
          </div>
          <p className="text-sm text-orange-100 mb-3">{currentQuestion.question}</p>
