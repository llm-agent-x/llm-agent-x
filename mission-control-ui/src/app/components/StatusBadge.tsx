...
} from "lucide-react"; // Import MessageCircleQuestion
...
export const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-medium text-white ${config.color}`}
    >
      {config.icon}
